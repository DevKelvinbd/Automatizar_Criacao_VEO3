...
if date.today() >= EXPIRATION_DATE:
    _msg = "Sua licença expirou em 10/05/2026.\n\nEntre em contato para renovar."
    try:
        import tkinter as tk
        from tkinter import messagebox
        root = tk.Tk()
        root.withdraw()
        messagebox.showerror("VEO3 — Licença Expirada", _msg)
        root.destroy()
    except Exception:
        try:
            # Windows nativo (funciona mesmo sem console e sem tkinter)
            import ctypes
            ctypes.windll.user32.MessageBoxW(0, _msg, "VEO3 — Licença Expirada", 0x10)
        except Exception:
            pass
    sys.exit(1)
